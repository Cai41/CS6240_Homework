...
public class Preprocessor {
	...
	static {
		...
		replacePattern = Pattern.compile("&", Pattern.LITERAL);
		try {
	...
	public static List<Tuple2<String, List<String>>> map(String value) {
		ArrayList<Tuple2<String, List<String>>> res = new ArrayList<>();
		try {
			...
			html = replacePattern.matcher(html).replaceAll(Matcher.quoteReplacement("&amp;"));
			// html = html.replace("&", "&amp;");
			...
			Tuple2<String, List<String>> t = new Tuple2<>(pageName, linkPageNames);
			res.add(t);
			for (String neighbour : linkPageNames) {
				res.add(new Tuple2<>(neighbour, null));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
